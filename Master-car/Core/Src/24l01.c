...
uint8_t RX_ADDRESS[RX_ADR_WIDTH]={0xFF,0xFF,0xFF,0xFF,0x01}; //发送地址
...
uint8_t NRF24L01_RxPacket(uint8_t *rxbuf)
{
    ...
    if (sta & RX_OK)                                 //接收到数据
    {
        NRF24L01_Read_Buf(RD_RX_PLOAD, rxbuf, RX_PLOAD_WIDTH); //读取数据         //清除RX FIFO寄存器
...
void NRF24L01_RX_Mode(void)
{
    ...
    //NRF24L01_Write_Buf(NRF_WRITE_REG + TX_ADDR, (uint8_t *)RX_ADDRESS, RX_ADR_WIDTH); //写RX节点地址
